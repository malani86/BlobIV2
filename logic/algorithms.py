...
_DL_MODEL_CACHE = {}
DL_BRIGHT_PERCENTILE = 90
DL_MIN_OBJECT_SIZE = 16
DL_OPENING_RADIUS = 1
...
def rolling_ball_correction_rgb(image, radius=50):
    '''Matches the original UNet-DC RGB rolling-ball preprocessing.'''
...
def get_original_unetdc_model_class():
    '''Loads the original UNetDC class from unet-DC-segmentation-main if available.'''
    model_path = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__),
            "..",
            "..",
            "unet-DC-segmentation-main",
            "models",
            "model_2.py",
        )
    )
    if not os.path.exists(model_path):
        from logic.unetdc_model import UNetDC
        return UNetDC

    spec = importlib.util.spec_from_file_location("unetdc_original_model", model_path)
    if spec is None or spec.loader is None:
        from logic.unetdc_model import UNetDC
        return UNetDC
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.UNetDC

def segmentation_deep_learning(image, checkpoint_path, prob_thresh=0.3, background_radius=50, device="auto"):
    '''Segments image with a UNet-DC model
    Parameters:
    image : the image to segment
    checkpoint_path : path to the model checkpoint
    prob_thresh : threshold on sigmoid output
    background_radius : rolling-ball background correction radius
    device : "auto", "cpu", or "cuda"
    Returns:
    mask : a binary image'''
    if not checkpoint_path:
        raise ValueError("Please provide a checkpoint path for deep learning segmentation.")
    ...
    try:
        ...
        raise ValueError(
            "PyTorch is required for deep learning segmentation. Please install 'torch' first."
        ) from e

    if device not in ("auto", "cpu", "cuda"):
        raise ValueError("Deep learning device must be 'auto', 'cpu', or 'cuda'.")
    if device == "cuda" and not torch.cuda.is_available():
        raise ValueError("CUDA was selected but no CUDA device is available.")

    torch_device = "cuda" if (device == "auto" and torch.cuda.is_available()) else ("cpu" if device == "auto" else device)
    model_key = (checkpoint_path, torch_device)
    model = _DL_MODEL_CACHE.get(model_key)
    if model is None:
        UNetDC = get_original_unetdc_model_class()
        ...
        _DL_MODEL_CACHE[model_key] = model

    image_array = np.asarray(image)
    if image_array.ndim == 2:
        image_rgb = np.stack((image_array, image_array, image_array), axis=-1)
        raw_intensity = image_array.astype(np.float32, copy=False)
    elif image_array.ndim == 3 and image_array.shape[2] == 4:
        image_rgb = (color.rgba2rgb(image_array) * 255).astype(np.uint8)
        raw_intensity = color.rgb2gray(image_rgb).astype(np.float32, copy=False)
    elif image_array.ndim == 3 and image_array.shape[2] >= 3:
        image_rgb = image_array[:, :, :3]
        raw_intensity = color.rgb2gray(image_rgb).astype(np.float32, copy=False)
    else:
        raise ValueError("Unsupported image shape for deep learning segmentation.")

    if image_rgb.dtype != np.uint8:
        image_rgb = image_rgb.astype(np.float32, copy=False)
        if np.max(image_rgb) <= 1.0:
            image_rgb *= 255.0
        image_rgb = np.clip(image_rgb, 0, 255).astype(np.uint8)

    original_h, original_w = image_rgb.shape[:2]
    corrected = rolling_ball_correction_rgb(image_rgb, background_radius)
    resized_image = cv2.resize(corrected, (512, 512), interpolation=cv2.INTER_AREA)
    resized_image = resized_image.astype(np.float32) / 255.0
    tensor = torch.from_numpy(resized_image).permute(2, 0, 1).unsqueeze(0).to(torch_device)

    with torch.no_grad():
        probs = model(tensor)
        mask_512 = (probs > prob_thresh).squeeze().cpu().numpy().astype(np.uint8)

    mask = cv2.resize(mask_512, (original_w, original_h), interpolation=cv2.INTER_NEAREST).astype(bool)
    # Heuristic tuning for BlobInspector: keep the DL support region but retain only
    # the brightest raw pixels so the result better matches sparse droplet masks.
    bright_threshold = np.percentile(raw_intensity, DL_BRIGHT_PERCENTILE)
    mask = mask & (raw_intensity >= bright_threshold)
    mask = remove_small_objects(mask, min_size=DL_MIN_OBJECT_SIZE)
    if DL_OPENING_RADIUS > 0:
        mask = binary_opening(mask, footprint=disk(DL_OPENING_RADIUS))
    return mask
